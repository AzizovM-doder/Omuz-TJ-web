...
export default function CrmProductPage({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans selection:bg-purple-500/30">
       ...
       {/* Custom CRM Request Form */}
       <section className="py-24 lg:py-32 relative overflow-hidden bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800">
           {/* Ambient Tech Glows */}
           <div className="absolute top-0 right-1/4 w-[600px] h-[600px] bg-purple-500/10 dark:bg-purple-500/5 rounded-full blur-[100px] pointer-events-none" />
           <div className="absolute bottom-0 left-1/4 w-[600px] h-[600px] bg-sky-500/10 dark:bg-sky-500/5 rounded-full blur-[100px] pointer-events-none" />
           <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-[0.03] pointer-events-none mix-blend-multiply dark:mix-blend-screen" />

           <div className="container max-w-7xl mx-auto px-4 relative z-10">
               <div className="bg-slate-50 dark:bg-slate-900/40 backdrop-blur-3xl border border-slate-200 dark:border-slate-800 rounded-[2.5rem] sm:rounded-[3rem] p-8 sm:p-12 lg:p-16 shadow-2xl overflow-hidden relative">
                   {/* Decorative border glow */}
                   <div className="absolute inset-0 ring-1 ring-inset ring-purple-500/20 dark:ring-purple-500/10 rounded-[2.5rem] sm:rounded-[3rem] pointer-events-none" />
                   ...
                   <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center relative z-10">
                       ...
                       >
                           ...
                           <p className="text-slate-600 dark:text-slate-400 text-lg mb-10 font-light">
                               {formT("subtitle")}
                           </p>

                           <form onSubmit={handleFormSubmit} className="space-y-6">
                               <div className="space-y-2">
                                   <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{formT("name_placeholder")}</label>
                                   <Input 
                                       placeholder="e.g. John Doe"
                                       required
                                       className="h-14 rounded-2xl bg-white dark:bg-slate-950/50 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-purple-500/50 transition-all text-lg shadow-sm"
                                   />
                               </div>
                               <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                   <div className="space-y-2">
                                       <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{formT("email_placeholder")}</label>
                                       <Input 
                                           type="email"
                                   ...
                                   <div className="space-y-2">
                                       <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{formT("phone_placeholder")}</label>
                                       <Input 
                                           type="tel"
                               ...
                               <Button 
                                   ...
                                   className="w-full h-16 rounded-2xl bg-purple-600 hover:bg-purple-700 text-white text-lg font-bold shadow-[0_0_40px_-10px_rgba(168,85,247,0.8)] transition-all hover:scale-[1.02] mt-8 group"
                               >
                                   {isSubmitting ? (
                                       <><Loader2 className="mr-2 h-6 w-6 animate-spin" /> {formT("sending")}</>
                                   ) : (
                                       <>{formT("submit")} <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" /></>
                                   )}
                               </Button>
                           </form>
                       </motion.div>
                       ...
                       >
                           ...
                           <div className="relative w-full max-w-[500px] aspect-square transform perspective-[1000px] rotate-y-[-10deg] rotate-x-[5deg]">
                               <div className="absolute inset-0 bg-gradient-to-tr from-purple-500 to-sky-500 rounded-3xl blur-2xl opacity-20 animate-pulse" />
                               
                               <Image 
                                   src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=2070&auto=format&fit=crop"
                                   alt="System Preview"
                                   fill
                                   className="object-cover rounded-3xl shadow-2xl ring-1 ring-slate-900/5 dark:ring-white/10"
                               />
                               
                               <motion.div 
                                  animate={{ y: [-15, 15, -15] }}
                                  transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
                                  className="absolute -top-8 -right-8 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700"
                               >
                                  <BarChart3 className="w-10 h-10 text-purple-600" />
                                  <div className="h-2 w-16 bg-slate-200 dark:bg-slate-700 rounded-full mt-4" />
                               </motion.div>

                               <motion.div 
                                  animate={{ y: [10, -10, 10] }}
                                  transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }}
                                  className="absolute -bottom-8 -left-8 bg-sky-500 text-white p-6 rounded-2xl shadow-xl"
                               >
                                  <Users className="w-10 h-10" />
                                  <div className="h-2 w-16 bg-white/30 rounded-full mt-4" />
                               </motion.div>
                           </div>
                       </motion.div>
