...
export default function AboutPage({
  ...
  const values = [
    { key: 'innovation', icon: Lightbulb, color: "text-yellow-500", bg: "bg-yellow-100 dark:bg-yellow-900/20" },
    { key: 'community', icon: Users, color: "text-sky-500", bg: "bg-sky-100 dark:bg-sky-900/20" },
    { key: 'quality', icon: Shield, color: "text-purple-500", bg: "bg-purple-100 dark:bg-purple-900/20" }
  ];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
       ...
       {/* Stats Section */}
       <div className="-mt-20 relative z-20 container max-w-7xl mx-auto px-4 mb-24">
       ...
       <section className="py-24 bg-slate-50 dark:bg-slate-950">
          <div className="container max-w-7xl mx-auto px-4">
            <h2 className="text-4xl font-bold text-center mb-16 tracking-tight">{t('values_title')}</h2>
            <div className="grid md:grid-cols-3 gap-8">
               {values.map((val) => (
                 <div key={val.key} className="bg-background border border-border p-8 rounded-3xl hover:shadow-xl transition-all duration-300 group">
                    <div className={`w-14 h-14 ${val.bg} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}>
                       <val.icon className={`w-7 h-7 ${val.color}`} />
                    </div>
                    <h3 className="text-2xl font-bold mb-4">{t(`values.${val.key}`)}</h3>
       ...
       {/* Our Team (Simplified Placeholder utilizing translation keys) */}
          ...
          <div className="grid md:grid-cols-4 gap-8">
             {[1, 2, 3, 4].map((i) => (
                <div key={i} className="group relative overflow-hidden rounded-3xl aspect-[3/4]">
                   <Image 
                     src={`https://images.unsplash.com/photo-${1500000000000 + i}?q=80&w=800&auto=format&fit=crop`} // Placeholder logic
                     alt="Team Member"
                     fill
       ...
       <section className="py-24 container max-w-7xl mx-auto px-4 text-center">
          <div className="bg-gradient-to-br from-sky-600 to-purple-600 rounded-3xl p-12 md:p-24 text-white shadow-2xl relative overflow-hidden">
             ...
             <div className="relative z-10 max-w-3xl mx-auto">
                ...
                <Button asChild size="lg" className="bg-white text-sky-600 hover:bg-sky-50 rounded-full px-10 h-14 text-lg font-bold shadow-lg hover:shadow-xl transition-all">
